...
<head>
    ...
    <title>@yield('title', 'Vendor — Shipment Otomotif')</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .main-header {
            ...
            z-index: 1030; box-shadow: 0 1px 3px rgba(0,0,0,.08);
        ...
        .dropdown-menu-adminlte {
            ...
            box-shadow: 0 2px 12px rgba(0,0,0,.15); min-width: 200px; z-index: 1050; padding: 4px 0;
        }
        ...
        .main-sidebar {
            ...
            z-index: 1035; overflow-x: hidden;
            transition: width .3s ease;
            display: flex; flex-direction: column;
        }
        .brand-link {
            display: flex; align-items: center; gap: 10px;
            padding: 12px 16px; height: var(--navbar-height);
            border-bottom: 1px solid rgba(255,255,255,.1);
            color: #fff !important; font-weight: 700; font-size: 16px; white-space: nowrap;
        }
        .brand-link i { font-size: 22px; }
        .sidebar { padding: 8px 0; overflow-y: auto; flex: 1; }
        .user-panel {
            display: flex; align-items: center; gap: 10px;
            padding: 12px 16px; border-bottom: 1px solid rgba(255,255,255,.1); margin-bottom: 8px;
        }
        ...
        .nav-header {
            padding: 6px 16px 4px; font-size: 11px; font-weight: 700; letter-spacing: .8px;
            color: rgba(255,255,255,.4); text-transform: uppercase;
        }
        .nav-sidebar { list-style: none; padding: 0; margin: 0; }
        .nav-sidebar .nav-link {
            display: flex; align-items: center; gap: 10px;
            padding: 10px 16px; color: var(--sidebar-color);
            transition: background .2s, color .2s; white-space: nowrap; font-size: 14px;
        }
        .nav-sidebar .nav-link:hover { background: var(--sidebar-hover-bg); color: #fff; }
        .nav-sidebar .nav-link.active {
            background: var(--sidebar-active-bg); color: #fff;
            box-shadow: 2px 0 8px rgba(23,162,184,.4);
        }
        ...
        .content-wrapper {
            ...
            background: var(--content-bg); transition: margin-left .3s ease;
        ...
        .main-footer {
            padding: 14px 24px;
            background: var(--navbar-bg); border-top: 1px solid var(--navbar-border);
        ...
        /* CARD */
        .card {
            background: var(--card-bg); border-radius: 6px; box-shadow: var(--card-shadow);
            margin-bottom: 20px; border: 0;
        }
        .card-header {
            display: flex; align-items: center; justify-content: space-between;
            padding: 12px 16px; border-bottom: 1px solid rgba(0,0,0,.06); background: transparent;
        }
        .card-title { font-size: 15px; font-weight: 600; color: #343a40; margin: 0; }
        .card-body { padding: 16px; }
        .card-footer { padding: 10px 16px; border-top: 1px solid rgba(0,0,0,.06); background: rgba(0,0,0,.02); font-size: 13px; color: #6c757d; }
        ...
        .table th { padding: 10px 14px; background: #f8f9fa; border-bottom: 2px solid #dee2e6; font-weight: 600; color: #495057; text-align: left; }
        ...
        .table-hover tbody tr:hover { background: rgba(23,162,184,.04); }
        ...
        /* BADGE */
        .badge { display: inline-block; padding: 3px 8px; font-size: 11px; font-weight: 600; border-radius: 10px; color: #fff; }
        ...
        .alert .btn-close-alert { background: none; border: none; cursor: pointer; color: inherit; opacity: .6; font-size: 18px; padding: 0; }
        ...
        .form-control:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(23,162,184,.15); }

        /* GRID */
        .row { display: flex; flex-wrap: wrap; margin: 0 -12px; }
        [class^="col-"], [class*=" col-"] { padding: 0 12px; }
        .col-3 { width: 25%; } .col-4 { width: 33.33%; } .col-6 { width: 50%; }
        .col-8 { width: 66.66%; } .col-12 { width: 100%; }
        ...
        @media (max-width: 992px) {
            ...
            .col-6 { width: 100%; }
            .sidebar-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.5); z-index: 1034; display: none; }
        ...
        div.dt-search input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(17,162,184,.1); }
...
<body>
    <div class="wrapper">
        ...
        <nav class="main-header navbar">
            <div class="navbar-left">
                <ul class="navbar-nav">
                    <li><a class="nav-link" id="sidebarToggle" role="button"><i class="fas fa-bars"></i></a></li>
                </ul>
            </div>
            ...
            <div class="sidebar">
                ...
                <nav>
                    <ul class="nav-sidebar">
                        <li class="nav-item">
                            <a href="{{ route('vendor.dashboard') }}" class="nav-link @if(request()->routeIs('vendor.dashboard')) active @endif">
                                <i class="nav-icon fas fa-tachometer-alt"></i>
                                <p>Dashboard</p>
                            </a>
                        </li>
                        @if(auth()->user()->vendor && auth()->user()->vendor->position)
                        <li class="nav-item">
                            <a href="{{ route('vendor.scanner') }}" class="nav-link @if(request()->routeIs('vendor.scanner*')) active @endif">
                                <i class="nav-icon fas fa-qrcode"></i>
                                <p>Scan VIN</p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="{{ route('vendor.history') }}" class="nav-link @if(request()->routeIs('vendor.history')) active @endif">
                        ...
                        <li class="nav-item">
                            <a href="{{ route('vendor.docs.user-guide') }}" class="nav-link @if(request()->routeIs('vendor.docs.user-guide')) active @endif">
    ...
    (function() {
        ...
        document.addEventListener('click', function(e) {
            document.querySelectorAll('.dropdown.show').forEach(dd => { if (!dd.contains(e.target)) dd.classList.remove('show'); });
        });
    })();
    </script>
